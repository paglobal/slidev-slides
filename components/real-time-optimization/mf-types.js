...
function mf_trapezoidal(input, left, maxLeft, maxRight, right) {
  if (input < left || input > right) return 0;
  if (left <= input && input <= maxLeft)
    return (input - left) / (maxLeft - left);
...
function mf_sigmoid(input, steepness, transitionCoordinate) {
  return input === transitionCoordinate
    ? 0.5
...
export {
  mf_triangle as triangle,
  mf_trapezoidal as trapeze,
  mf_gaussian as gauss,
  mf_sigmoid as sigma,
  mf_singleton as singleton,
};
